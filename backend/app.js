...
app.post("/api/unospacijenta", async (request, response) => {
  ...
  pacijent = [[data.ime, data.prezime, data.dob, data.spol, data.user]]
  const db = getDB();
  try {
    const [results] = await db.query("INSERT INTO PACIJENT (Ime_pacijent, Prezime_pacijent, DOB_Pacijent, Spol_pacijent, id_user) VALUES ?", [pacijent]);
